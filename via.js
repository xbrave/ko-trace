(function(window) {
	...
	var _ = {};
	var params = {};
	var api_host = "http://172.16.8.194:8897/api/v1/entity/exists-entity-name";
	var enterTime = null;
	...
	function _init() {
		// 判断token是否存在，如果不在则结束
		params.code =
			'cxKLOpJSs8OajOTXOs+ROCOQ1G8InxjrSmOQGL4lV0BeiXrgRVzb8C+q4jCNX1VI8sB1Q2hUYBBKr72hajrzHhG8tG9pegfb0MH1k2gSB3qyQCGxjoBhIRKCRCj2ziEmOm14ewVmGs2Zn4IMl2l3nXVxZ/tBznRY4FWtvkcAwfEdUG7SqSYQdol9ijNZeGvC';
	...
	_maq.trigger = function(arr, e) {
		...
		switch (arr[0]) {
			...
			case '_pageview':
				...
				p2.user_id = params.user_id;
				p2.source = '';
				_send(_serilize(p2));
                return false;
            case '_downLoad':
                    params.triggerType = 'downLoad';
                    var p3 = params;
                    arr.shift();
                    p3.source = args[0];
                    p3.extraInfo = '';
                    //debugger;
                    p3.event = 'downLoad';
                    p3.user_id = params.user_id;
                    p3.source && _send(_serilize(p1));
                    //  delete p1.source;
                    //  delete p1.event;
                    // return false
                    break;
